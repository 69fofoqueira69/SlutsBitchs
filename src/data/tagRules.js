...
export function calculateAgeData(ageValue) {
  const match = ageRules.find((rule) => inRange(ageValue, rule.min, rule.max));
  return match
    ? { value: ageValue, tag: match.tag, range: match.range, emoji: match.emoji }
    : { value: ageValue, tag: '', range: '', emoji: '' };
}

export function calculateMeasurementData(type, value) {
  const match = (measurementRules[type] || []).find((rule) => inRange(value, rule.min, rule.max));
  return match
    ? { value, tag: match.tag, range: match.range, emoji: match.emoji }
    : { value, tag: '', range: '', emoji: '' };
}
