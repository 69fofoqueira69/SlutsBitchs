...
function listarMidias(midia) {
  const imagens = (midia?.imagens || []).filter(srcValido).map((src) => ({ tipo: 'imagem', src }));
  const gifs = (midia?.gifs || []).filter(srcValido).map((src) => ({ tipo: 'gif', src }));
  return [...imagens, ...gifs];
}
